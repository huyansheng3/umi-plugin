...
var extendMethod = {
    ...
    __restkey: function () {
        return (this.url.match(/{\w+}/g) || [])
            .map(function (_) { return _.replace(/{|}/g, ""); })
            .concat(["...restData"])
            .join(", ");
    },
    /** 判断请求方式 */
    ...
    if (fs_1.existsSync(apiPath)) {
        ...
        for (var i = 0; i < localePaths.length; i++) {
            ...
            if (stats.isFile() && fileInfo) {
                try {
                    api = __assign(__assign({}, api), js_yaml_1.safeLoad(fs_1.readFileSync(fullname, "utf-8")));
                }
                catch (e) {
...
function getServices(apiPath) {
    ...
    return Object.keys(serviceApi).reduce(function (pre, key) {
        return __spreadArrays(pre, [__assign({ key: key }, serviceApi[key])]);
    }, []);
}
...
function default_1(api, options) {
    ...
    api.onGenerateFiles(function () {
        ...
        var wrapperTpl = fs_1.readFileSync(path_1.join(__dirname, "../template/" + tplfile + ".mustache"), "utf-8");
        var wrapperContent = mustache_1.render(wrapperTpl, __assign({ services: services }, extendMethod));
